...
import { requireGroup } from '../auth/auth'
...
export async function usersLoader({ request }: { request: Request }): Promise<User[]> {
  return authenticatedApi(request).users.list(request.signal)
}

export function createUserLoader({ request }: { request: Request }) { requireGroup(request, 'Admins'); return null }

export async function usersAction({ request }: { request: Request }) {
  const api = authenticatedApi(request)
  const input = inputFrom(await request.formData())
  assertValid(input)
  const user = await api.users.create(input)
  return redirect(`/users/${user.id}`)
}

export async function userLoader({ params, request }: { params: { userId?: string }; request: Request }) {
  const user = await authenticatedApi(request).users.get(params.userId ?? '', request.signal)
...
export async function userAction({ params, request }: { params: { userId?: string }; request: Request }) {
  ...
  const input = inputFrom(await request.formData())
  assertValid(input)
  const user = await api.users.update(id, input)
  if (!user) throw new Response('ユーザーが見つかりません。', { status: 404 })
  return redirect(`/users/${user.id}`)
}
...
export function UserListPage() {
  ...
  return (
    <main className="page">
      <div className="page-heading"><div><p className="eyebrow">GET /users</p><h1>ユーザー一覧</h1><p>この画面の loader が 550ms の API 待機を再現します。</p></div>{currentUser.groups.includes('Admins') && <Link className="button" to="/users/new">＋ ユーザーを追加</Link>}</div>
...
export function UserDetailPage() {
  ...
  const canEdit = useOutletContext<AuthUser>().groups.includes('Admins')
  return <main className="page narrow"><div className="page-heading"><div><p className="eyebrow">GET /users/{user.id.slice(0, 8)}</p><h1>{user.name}</h1><p>最終更新: {new Date(user.updatedAt).toLocaleString('ja-JP')}</p></div><Link to="/users">← 一覧へ</Link></div><div className="loader-note"><code>userLoader</code><span>→ useLoaderData (1 record)</span></div>{canEdit ? <><Form id="edit-user" className="user-form" method="patch"><UserFields user={user} /></Form><div className="form-actions detail-actions"><Form method="delete" className="inline-form"><button className="danger" type="submit">削除</button></Form><SubmitButton form="edit-user">変更を保存</SubmitButton></div></> : <div className="user-form"><UserFields user={user} disabled /><p className="security-note">Viewer は閲覧のみです。変更は API 側でも拒否されます。</p></div>}</main>
}
